terraform {
  ...
  backend "azurerm" {
    ...
    key                  = "tricor-landing-zone/tricor-SITE-lz-ENV.tfstate" # TODO: UPDATE FOR NEW SUBSCRIPTION
  }
}
...
provider "azurerm" {
  features {}
  subscription_id = data.terraform_remote_state.subscriptions.outputs.subscription-landingzone_SITE_ENV-id # TODO: UPDATE FOR NEW SUBSCRIPTION
}
...
provider "azurerm" {
  alias = "landingzone_SITE_ENV" # TODO: Update site and environment
  features {}
